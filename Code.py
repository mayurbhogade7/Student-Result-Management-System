...
engine = sqlalchemy.create_engine(
    'mysql+pymysql://root:root@localhost/student')

while True:
print("  ----------WELCOME  TO  STUDENT  RESULT  MANAGEMENT  ANALYSIS-----------")
print("OPERATIONS:-")
print("1.Show  All  Students  Name  and  Roll  Number  List  of  Class  A  and  Class  B")
print("2.Show  All  Students  Marks  List  of  Class  A  and  Class  B")
print("3.Show  Average  Marks  Of  Students  In  Each  Subject")
print("4.Show  Highest  Scores  In  Each  Subject") print("5.Show  Lowest  Scores  In  Each  Subject")
print("6.Show  Subject  Wise  Pass  Percentage")
print("7.Highest  and  Lowest  Scorer  From  Class  A  and  Class  B")
print("8.Show  result  of  particular  student")
print("9.Exit")
print('\n')
choice = input("Enter  choice  of  operation  want  to  perform:  ")
print('\n')
...
if (choice == '3'):
print("\n*****Average  Marks  Class  A*****")

avga1 = data2.TOC.mean()
...
plt.title("DBMS  Subject(Class  A)")
plt.xlabel("Roll_No")
plt.ylabel("Marks")
d2['DBMS'].plot.bar()
plt.show()
...
plt.title("SEPM  Subject(Class  A)")
plt.xlabel("Roll_No")
plt.ylabel("Marks")
d3['SEPM'].plot.bar()
plt.show()
...
plt.title("ISEE  Subject(Class  A)")
plt.xlabel("Roll_No")
plt.ylabel("Marks")
d4['ISEE'].plot.bar()
plt.show()
...
print(d5[['Name',  'CN']].reset_index(drop=True))
plt.title("CN  Subject(Class  A)")
...
plt.title("DBMS  Subject(Class  B)")
plt.xlabel("Roll_No")
plt.ylabel("Marks")
d7['DBMS'].plot.bar()
plt.show()
...
plt.title("SEPM  Subject(Class  B)")
plt.xlabel("Roll_No")
plt.ylabel("Marks")
d8['SEPM'].plot.bar()
plt.show()
...
plt.title("ISEE  Subject(Class  B)")
plt.xlabel("Roll_No")
plt.ylabel("Marks")
d9['ISEE'].plot.bar()
plt.show() print('\n')
...
toc_pass = df1.query('Result=="Pass"').Result.count()
print("Number  of  students  passed  in  TOC:",  toc_pass)
roll_count1 = df1["Roll_No"].count()
#  print(roll_count1)
per_a_toc = (toc_pass / (roll_count1)) * 100
print("Percentage  of  pass  stdents  in  TOC  of  div  A:",  per_a_toc)

labels = 'Passed',  'Failed'
sizes = [per_a_toc, 100-per_a_toc]
colors = ['green',  'red']
explode = (0,  0)
plt.pie(sizes,  explode=explode,  labels=labels, colors=colors,
        autopct='%1.1f%%',  shadow=True,  startangle=140)
plt.axis('equal')
plt.title("TOC  Subject")
plt.show()
...
dbms_pass = df2.query('Result=="Pass"').Result.count()
print("Number  of  students  passed  in  DBMS:",  dbms_pass)
roll_count2 = df2["Roll_No"].count()
#  print(roll_count2)
...
sepm_pass = df3.query('Result=="Pass"').Result.count()
print("Number  of  students  passed  in  SEPM:",  sepm_pass)
roll_count3 = df3["Roll_No"].count() print(roll_count3)
...
isee_pass = df4.query('Result=="Pass"').Result.count()
print("Number  of  students  passed  in  ISEE:",  isee_pass)
roll_count4 = df4["Roll_No"].count()
...
plt.pie(sizes3,  explode=explode,  labels=labels,  colors=colors,
        autopct='%1.1f%%',  shadow=True, startangle=140)
plt.axis('equal')
plt.title("ISEE  Subject")
plt.show()

df5 = data.filter(["Roll_No",  'Name',  'CN'],  axis=1)
Result = []
for v in df5['CN']:
if v >= 12:
Result.append("Pass")
...
Result.append("Fail")
df5["Result"] = Result print(df5)
cn_pass = df5.query('Result=="Pass"').Result.count()
print("Number  of  students  passed  in  CN:",  cn_pass)
roll_count5 = df5["Roll_No"].count()
#  print(roll_count5)
...
explode = (0,  0)
plt.pie(sizes4,  explode=explode,  labels=labels,  colors=colors,
        autopct='%1.1f%%',  shadow=True, startangle=140)
plt.axis('equal')
plt.title("CN  Subject")
plt.show()

per_a_bar = [per_a_toc, per_a_dbms, per_a_sepm, per_a_isee, per_a_cn]
sub = ['TOC', 'DBMS', 'SEPM', 'ISEE', 'CN']
index = np.arange(len(sub))
plt.bar(sub,  per_a_bar)
plt.xlabel('Subject')
plt.ylabel('Passing  Percentage')

plt.title('Passing  Percentage  Of  All  Subjects')
plt.show()

elif(h2 == 'b' or h2 == 'B'):
df6 = data1.filter(["Roll_No",  'Name',  'TOC'],  axis=1)
Result = []
for v in df6['TOC']:
if v >= 12:
Result.append("Pass")
...
Result.append("Fail")
df6["Result"] = Result print(df6)
toc_pass1 = df6.query('Result=="Pass"').Result.count()
print("Number  of  students  passed  in  TOC:",  toc_pass1)
roll_count6 = df6["Roll_No"].count()
#  print(roll_count1)
per_b_toc = (toc_pass1 / (roll_count6)) * 100
print("Percentage  of  pass  stdents  in  TOC  of  div  B:",  per_b_toc)

labels = 'Passed',  'Failed'
sizes5 = [per_b_toc,  100 - per_b_toc] colors = ['green',  'red']
explode = (0,  0)
plt.pie(sizes5,  explode=explode,  labels=labels,  colors=colors,
        autopct='%1.1f%%',  shadow=True, startangle=140)
plt.axis('equal')
plt.title("TOC  Subject")
plt.show()
...
Result.append("Fail")
df7["Result"] = Result print(df7)
dbms_pass1 = df7.query('Result=="Pass"').Result.count()
print("Number  of  students  passed  in  DBMS:",  dbms_pass1)
roll_count7 = df7["Roll_No"].count()
#  print(roll_count2)
...
explode = (0,  0)
plt.pie(sizes6,  explode=explode,  labels=labels,  colors=colors,
        autopct='%1.1f%%',  shadow=True, startangle=140)
plt.axis('equal')
plt.title("DBMS  Subject")
plt.show()
...
Result.append("Fail")
df8["Result"] = Result print(df8)
sepm_pass1 = df8.query('Result=="Pass"').Result.count()
print("Number  of  students  passed  in  SEPM:",  sepm_pass1)
roll_count8 = df8["Roll_No"].count()
#  print(roll_count3)
...
explode = (0,  0)
plt.pie(sizes7,  explode=explode,  labels=labels,  colors=colors,
        autopct='%1.1f%%',  shadow=True, startangle=140)
plt.axis('equal')
plt.title("SEPM  Subject")
plt.show()
...
isee_pass1 = df9.query('Result=="Pass"').Result.count()
print("Number  of  students  passed  in  ISEE:",  isee_pass1)
roll_count9 = df9["Roll_No"].count()
#  print(roll_count4)
...
explode = (0,  0)
plt.pie(sizes8,  explode=explode,  labels=labels,  colors=colors,
        autopct='%1.1f%%',  shadow=True, startangle=140)
plt.axis('equal')
plt.title("ISEE  Subject")
plt.show()
...
cn_pass1 = df10.query('Result=="Pass"').Result.count()
print("Number  of  students  passed  in  CN:",  cn_pass1)
roll_count10 = df10["Roll_No"].count()
#  print(roll_count5)
per_b_cn = (cn_pass1 / (roll_count10)) * 100
print("Percentage  of  pass  stdents  in  CN  of  div  B:",  per_b_cn)

labels = 'Passed',  'Failed'
sizes9 = [per_b_cn,  100 - per_b_cn] colors = ['green',  'red']
explode = (0,  0)
plt.pie(sizes9,  explode=explode,  labels=labels,  colors=colors,
        autopct='%1.1f%%',  shadow=True, startangle=140)
plt.axis('equal')
plt.title("CN  Subject")
plt.show()

per_a_bar = [per_b_toc,  per_b_dbms,  per_b_sepm,  per_b_isee,  per_b_cn]
sub = ['TOC',  'DBMS',  'SEPM',  'ISEE',  'CN']
index = np.arange(len(sub))
plt.bar(sub,  per_a_bar)
plt.xlabel('Subject')
...
#  Make  the  plot
plt.bar(r1,  bars1,  color='green',  width=barWidth,
        edgecolor='white', label='Highest  Score')
plt.bar(r2,  bars2,  color='red',  width=barWidth,
        edgecolor='white', label='Lowest  Score')

#  Add  xticks  on  the  middle  of  the  group  bars
plt.xlabel('Class')
plt.xticks([r + barWidth for r in range(len(bars1))],  ['A',  'B']) plt.ylabel('Marks')ncol = 1)

#  Create  legend  &  Show  graphic
plt.legend(loc = 'upper  center',  bbox_to_anchor = (1.45,  0.8),  shadow = True,
plt.title('Highest  and  Lowest  Score')
plt.show()
...
per.insert(4,  data2['CN'][n - 1]) plt.bar(y_pos,  per,  align='center',  alpha=0.5)
